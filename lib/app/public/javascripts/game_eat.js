window.onload = function () {
	alert("In this game, you will have to eat as much food as possible in order to survive. Hold the up and down keys in order to move around and catch the food. If you do not get enough food, you will lose. Press 'Ok' to start playing.");
}
...
function draw()
{
	...
	else
	{
		...
		if (score_num >= 500)
		{
			...
			window.location = "http://localhost:4567/herb_choice_F";
		}
		else
		{
			alert("You have failed to eat enough food, so you shall die!");
			window.location = "http://localhost:4567/herb_death";
		}
	}
}
...
function drawBg()
{
	...
	var drawY = 0;
	ctxBg.drawImage(spriteBg,srcX,srcY,game_width,game_height,drawX,drawY,game_width,game_height);
}
...
function drawScore()
{
	...
	ctxScore.fillText("Time: "+ time/10 ,10,50);
}
...
Rex.prototype.draw = function() 
{
	...
	this.checkKeys();
	if (this.isDownKey)
		ctxRex.drawImage(sprite_rex1,this.srcX+99,this.srcY+865,this.width,this.height,this.drawX+100,this.drawY,this.width,this.height);
	else if(this.isUpKey)
		ctxRex.drawImage(sprite_rex2,this.srcX+320,this.srcY+865,this.width,this.height,this.drawX+100,this.drawY,this.width,this.height);
	else
		ctxRex.drawImage(sprite_rex1,this.srcX+99,this.srcY+865,this.width,this.height,this.drawX+100,this.drawY,this.width,this.height);
};
...
Objects.prototype.draw = function() 
{
	clearObject();
	 ...
	 ctxObjects.drawImage(sprite_metor,this.srcX+550,this.srcY+909,this.width,this.height,this.drawX,this.drawY,this.width+30,this.height+10);
};
